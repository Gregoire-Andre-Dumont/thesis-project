"""Occlusion-length view of the live claim_1 run: coverage gain over the sam baseline against the number of
occluded frames, shown on all clips and again with the largest-area quartile dropped (that quartile is where sam
is already saturated and neither oracle has room to help, so it dilutes the trend).

The line is a single named commit threshold and the band is the full range across all of them, so band width
reads as threshold sensitivity. sam's own coverage is printed under each bin -- the gains are only interpretable against it."""
import sys
import json
import pickle
...
def binned(values, baseline, gain_a, gain_b, n_bins=4):
    edges = np.unique(np.quantile(values, np.linspace(0, 1, n_bins + 1)))
    index = np.clip(np.digitize(values, edges[1:-1]), 0, len(edges) - 2)
    return [(f"{int(edges[k])}-{int(edges[k+1])}", int((index == k).sum()), baseline[index == k].mean(),
             gain_a[index == k].mean(0), gain_b[index == k].mean(0))
            for k in range(len(edges) - 1)]


keep = areas < np.quantile(areas, 0.75)
panels = [("All clips", binned(occlusions, sam, memory_gain, mask_gain), len(rows)),
          ("Largest-area quartile dropped",
           binned(occlusions[keep], sam[keep], memory_gain[keep], mask_gain[keep]), int(keep.sum()))]

figure, axes = plt.subplots(1, 2, figsize=(13.5, 5.4), facecolor=SURFACE, sharey=True)
for axis, (title, bins, n_clips) in zip(axes, panels):
    axis.set_facecolor(SURFACE)
    x = np.arange(len(bins))
    for colour, column, label in ((MEMORY, 3, "memory oracle"), (MASK, 4, "mask oracle")):
    ...
    axis.set_xticks(x)
    axis.set_xticklabels([f"{b[0]}\nn={b[1]}\nsam {b[2]:.2f}" for b in bins], fontsize=9, color=INK2)
    axis.set_title(f"{title}   (n={n_clips})", fontsize=11, color=INK, pad=10, loc="left")
...
figure.suptitle(f"claim_1  occ[5,50]  ·  gain vs occlusion length  ·  line = commit threshold {thresholds[FEATURED]}, "
                f"shaded band = full range over thresholds {thresholds[0]}-{thresholds[-1]}", fontsize=12, color=INK, x=0.008, ha="left", y=0.985)
figure.tight_layout(rect=[0, 0, 1, 0.93])
figure.savefig("data/claim_1/occlusion_run.png", dpi=150, facecolor=SURFACE)
