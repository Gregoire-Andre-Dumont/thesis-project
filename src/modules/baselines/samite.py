...
class Samite:
    ...
    def __post_init__(self):
        ...
        model_config = self.model_config or "../conf/SAM2/samite_hiera_large.yaml"
        self.predictor = build_samite_video_predictor(model_config, self.checkpoint, device="cuda")
    ...
    def predict_masks(self, detection_data: DetectionData):
        ...
        args = SimpleNamespace(**{
            ...
            "max_num_indices": 30,         # paper: candidate memory pool size m=30
            "selection_strategy": "s2l_pos_feat_v2",
            "bias_mode": "kalman_pos",
            "bias_type": "v3",
            "use_prior_prompt": True,      # paper: PPG enabled for main results
            "alpha": 0.3,                  # paper: α=0.3 feature/position balance
            "beta": 0.7,                   # paper: cycle-consistency threshold
        })

        for frame_idx, object_ids, masks in self.predictor.propagate_in_video(inference_state, detection_data=detection_data, args=args):
